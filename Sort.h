...
void insertionSort( vector<Comparable> & a, int left, int right , Comparator less_than)
{
    for( int p = left + 1; p <= right; ++p )
    {
        ...
        for( j = p; j > left && less_than(tmp , a[ j - 1 ]); --j ) //added the less_than comparator
            a[ j ] = std::move( a[ j - 1 ] );
        a[ j ] = std::move( tmp );
    }
}
 ...
 */
template <typename Comparable, typename Comparator>
void percDown( vector<Comparable> & a, int i, int n, Comparator less_than ) //added comparator
{
    int child;
    Comparable tmp;
    ...
    {
        child = leftChild( i );
        if( child != n - 1 && less_than(a[ child ] , a[ child + 1 ] )) //added the comparator so the default is not purely less than
            ++child;
        if(less_than(tmp , a[ child ] )) //added less than comparator to remove default
...
void heapsort( vector<Comparable> & a, Comparator less_than )
{
    for( int i = a.size( ) / 2 - 1; i >= 0; --i )  /* buildHeap */
        percDown( a, i, a.size( ), less_than ); //added the comparator
    for( int j = a.size( ) - 1; j > 0; --j )
    {
        std::swap( a[ 0 ], a[ j ] );               /* deleteMax */
        percDown( a, 0, j, less_than ); //added a comparator
    }
}
 ...
 * less_than is the comparator passed through.
 */
template <typename Comparable, typename Comparator>
void merge( vector<Comparable> & a, vector<Comparable> & tmpArray,
            int leftPos, int rightPos, int rightEnd, Comparator less_than ) //added iterator
{
    ...
    while( leftPos <= leftEnd && rightPos <= rightEnd )
        if( less_than(a[ leftPos ] , a[ rightPos ]) )  //added the comparator so the default is not purely less than
...
void mergeSort( vector<Comparable> & a,
                vector<Comparable> & tmpArray, int left, int right , Comparator less_than   ) //added comparator
{
    if( left < right )
    {
...
void mergeSort( vector<Comparable> & a , Comparator less_than)
{
    ...
    mergeSort( a, tmpArray, 0, a.size( ) - 1 , less_than ); //added comparator
}
...
const Comparable & median3( vector<Comparable> & a, int left, int right , Comparator less_than)
{
    ...
    if(less_than(a[ center ], a[ left ]) )   //added the comparator to the three if's
...
void quicksort( vector<Comparable> & a, int left, int right, Comparator less_than )
{
    if( left + 10 <= right)  
    {
        const Comparable & pivot = median3( a, left, right, less_than ); //added comparator
        ...
        for( ; ; )
        {
            ...
            if( i < j ) //replaced default less than with comparator
                std::swap( a[ i ], a[ j ] );
            else
                break;
        }
        ...
        quicksort( a, left, i - 1 , less_than);     // Sort small elements + added comparator
        quicksort( a, i + 1, right , less_than );    // Sort large elements + added comparator
    }
    else  // Do an insertion sort on the subarray
        insertionSort( a, left, right , less_than); //added comparator
}
...
{
    quicksort( a, 0, a.size( ) - 1 , less_than);  //added less_than comparator
}
...
void quickSelect( vector<Comparable> & a, int left, int right, int k, Comparator less_than )
{
    if( left + 10 <= right )
    {
        ...
        for( ; ; )
        {
            while( less_than(a[ ++i ] , pivot )) { } //implemented the comparator below
            while( less_than(pivot , a[ --j ] )) { }
            if( less_than(i , j)) //replaced default comparator 
                std::swap( a[ i ], a[ j ] );
            else
                break;
        }
        ...
        if( k <= i )
            quickSelect( a, left, i - 1, k , less_than); //added comparators below
        else if( k > i + 1 )
